...
def handle_pairing_request(runtime: Any, request: PairingRequest) -> dict[str, Any]:
    ...
    if duplicate is not None:
        if duplicate.secret == request.proposed_secret:
            return pairing_response(runtime, duplicate, request)
    ...
    return pairing_response(runtime, pending, request)


def pairing_response(
    runtime: Any, pending: PendingPairing, request: PairingRequest
) -> dict[str, Any]:
    return {
        "approved": True,
