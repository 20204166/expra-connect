...
def _run_report(
    ...
    *,
    ...
    return {
        ...
        "observed_events": sorted(event_counts),
...
def analyze(inputs: list[str], *, full_ids: bool = False) -> dict[str, Any]:
    ...
    return {
        "format": "expra-connect-log-analysis/v2",
        "summary": {
            "files": len(files),
...
def _print_text(report: dict[str, Any]) -> None:
    ...
    print("=" * 26)
    print("Mode: schema-aware (missing role-specific fields are not failures)")
    print(f"Files: {summary['files']}")
    print(f"Events: {summary['total_events']}")
    print("Versions: " + (", ".join(summary["versions"]) or "none"))
    print("Local nodes: " + (", ".join(summary["local_node_ids"]) or "not reported"))
    ...
    for file_report in report["files"]:
        ...
        for run in file_report["runs"]:
            ...
            if start.get("discovery_started") is not None:
                discovery = str(start.get("discovery_started"))
            elif start.get("discovery_disabled") is True:
                discovery = "disabled"
            else:
                discovery = "not reported"

            print(f"    listener={listener} discovery={discovery}")
            print(
